...
- メタデータは all 固定に戻した（`-map_metadata 0`）
- GUI/CLI から上記設定を指定可能に変更
- GUI に「処理対象拡張子」のチェック群を追加（`.mp3 / .m4a / .aac / .flac / .wav / .ogg`）
- 既定のチェックは `.mp3 / .aac / .flac / .wav` をオン
- プレビュー/実行の対象ファイル列挙を選択拡張子に連動
- CLI に `--input-ext` を追加（複数指定可）
- メタデータ設定は `all` 固定に戻し、GUI/CLI からの選択を廃止
- 用途プロファイル機能を廃止し、音楽向け単一設定に整理
- 前段ノイズ低減/ノイズゲート機能を廃止
